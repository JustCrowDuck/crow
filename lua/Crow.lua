...
local function write_shot_num()
  ...
  client.log("Crow的hitlog: 目前已经开了 " .. shot_num .. " 枪了")
  chat_print.print(chat_print.vtbl, 0, 0, " \x0C[Crow的hitlog]  \x08目前已经开了 \x07" .. shot_num .. " \x08枪了")
end



client.add_callback("on_shot", function (shot_info)
    if not Crow_lua_loading or not menu.get_bool("Hitlog") then return end
   ...
   shot_num = shot_num + 1
   if results == "Hit" then
    chat_print.print(chat_print.vtbl, 0, 0, "[\x06Crow的大粪LW\x08] \x08第\x03" .. shot_num .. "\x08枪 蒙中了 \x03" .. targetname .. "\x08 造成了: \x07" .. serverdamage .. "\x08 部位 \x10" .. serverhitbox .. "\x08 | 命中率: \x09" .. hitchance .. "\x08 | 预计伤害: \x07" .. clientdamage .. "\x08 | 预计部位: \x10" .. clienthitbox .. "\x08 | 回溯: \x0C" .. backtrack .. "\x08 | 安全点: \x03" .. safe)
    
    elseif results == "Spread" then
        chat_print.print(chat_print.vtbl, 0, 0, "[\x02Crow的大粪LW\x08] \x08第\x03" .. shot_num .. "\x08枪 空枪 \x03" .. targetname .. "\x08 原因: \x02扩散\x08 | 命中率: \x09" .. hitchance .. "\x08 | 预计伤害: \x07" .. clientdamage .. "\x08 | 实际伤害: \x07" .. serverdamage .. "\x08 | 预计部位: \x10" .. clienthitbox .. "\x08 | 实际部位: \x10" .. serverhitbox .. "\x08 | 回溯: \x0C" .. backtrack .. "\x08 | 安全点: \x03" .. safe)
    
    elseif results == "Resolver" then
        chat_print.print(chat_print.vtbl, 0, 0, "[\x02Crow的大粪LW\x08] \x08第\x03" .. shot_num .. "\x08枪 空枪 \x03" .. targetname .. "\x08 原因: \x02解析\x08 | 命中率: \x09" .. hitchance .. "\x08 | 预计伤害: \x07" .. clientdamage .. "\x08 | 实际伤害:: \x07" .. serverdamage .. "\x08 | 预计部位: \x10" .. clienthitbox .. "\x08 | 实际部位: \x10" .. serverhitbox .. "\x08 | 回溯: \x0C" .. backtrack .. "\x08 | 安全点: \x03" .. safe)
    
    else
        chat_print.print(chat_print.vtbl, 0, 0, "[\x02Crow的大粪LW\x08] \x08第\x03" .. shot_num .. "\x08枪 空枪 \x03" .. targetname .. "\x08 原因: \x02死亡\x08 | 命中率: \x09" .. hitchance .. "\x08 | 预计伤害: \x07" .. clientdamage .. "\x08 | 实际伤害:: \x07" .. serverdamage .. "\x08 | 预计部位: \x10" .. clienthitbox .. "\x08 | 实际部位: \x10" .. serverhitbox .. "\x08 | 回溯: \x0C" .. backtrack .. "\x08 | 安全点: \x03" .. safe)
    end
...
events.register_event("player_death", function(event)
    if not Crow_lua_loading or not menu.get_bool("Killsay") then return end
...
events.register_event("player_hurt", function(event)
    if not Crow_lua_loading or not menu.get_bool("Hitsound") then return end
    local attacker = event:get_int("attacker")
    local attacker_to_player = engine.get_player_for_user_id(attacker)
    local local_index = engine.get_local_player_index()
